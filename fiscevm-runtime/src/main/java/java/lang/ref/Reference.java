...
public abstract class Reference<T> {
	...
	/*
	 * This class must be implemented by the vm vendor. The documented methods
	 * must be implemented to support the provided subclass implementations. As
	 * the provided subclass implementations are trivial and simply call
	 * initReference(Object) and initReference(Object, ReferenceQueue) from
	 * their constructors, the vm vendor may elect to implement the subclasses
	 * as well. Abstract class which describes behavior common to all reference
	 * objects.
	 */

	private native void register(Object referent);
	...
	/**
	 * Makes the referent {@code null}. This does not force the reference object
	 * to be enqueued.
	 */
	public void clear() {
		clear0();
	 ...
	 * 
	 * @return {@code true} if the {@code Reference} has been enqueued,
	...
	synchronized boolean enqueueImpl() {
		if (queue != null && !enqueued) {
			return this.enqueued = queue.enqueue(this);
		} else {
			return false;
		}
	}
