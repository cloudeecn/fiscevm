...
public class Introspector extends java.lang.Object {
	...
	private static Map<Class<?>, StandardBeanInfo> theCache = Collections
			.synchronizedMap(new HashMap<Class<?>, StandardBeanInfo>(
					DEFAULT_CAPACITY));
	 ...
	 * <code>Introspector</code> will ignore all <code>BeanInfo</code> class.</li>
	 * <li>If <code>flag==IGNORE_IMMEDIATE_BEANINFO</code>, the
	 * <code>Introspector</code> will ignore the <code>BeanInfo</code> class of
	 * the current bean class.</li>
	 * <li>If <code>flag==USE_ALL_BEANINFO</code>, the <code>Introspector</code>
	 * will use all <code>BeanInfo</code> class which have been found.</li>
	 * </ol>
	 * <p>
	 * The <code>Introspector</code> will cache the <code>BeanInfo</code>
	 * object. Subsequent calls to this method will be answered with the cached
