 ...
 * {@code application/x-www-form-urlencoded} MIME content type. It contains
 * helper methods used by the URI class, and performs encoding and decoding in a
...
class URIEncoderDecoder {
	...
	static void validate(String s, String legal) throws URISyntaxException {
		for (int i = 0; i < s.length();) {
			...
			if (ch == '%') {
				do {
					if (i + 2 >= s.length()) {
						throw new URISyntaxException(s, "" + //$NON-NLS-1$
								i);
					}
			...
			if (!((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z')
					...
					&& !Character.isSpaceChar(ch) && !Character
						.isISOControl(ch)))) {
				throw new URISyntaxException(s, "" + i); //$NON-NLS-1$
			}
			i++;
		}
	}
	 ...
	 * by '%'.
	 * <p>
	 * For example: '#' -> %23 Other characters, which are unicode chars that
	 * are not US-ASCII, and are not ISO Control or are not ISO Space chars, are
	 * preserved.
	 * <p>
	 ...
	 * {@code x-www-form-urlencoded} MIME content type using the UTF-8 encoding
	 * scheme.
	 * <p>
	...
	static String decode(String s) throws UnsupportedEncodingException {
		...
		for (int i = 0; i < s.length();) {
			...
			if (c == '%') {
				...
				do {
					if (i + 2 >= s.length()) {
						throw new IllegalArgumentException("" + i); //$NON-NLS-1$
					}
					int d1 = Character.digit(s.charAt(i + 1), 16);
					int d2 = Character.digit(s.charAt(i + 2), 16);
					if (d1 == -1 || d2 == -1) {
						throw new IllegalArgumentException(
								"" + s.substring(i, i + 3) + " " + //$NON-NLS-1$
										String.valueOf(i));
					}
