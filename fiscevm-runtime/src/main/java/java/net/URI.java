...
public final class URI implements Comparable<URI>, Serializable {
	...
	public URI(String scheme, String userinfo, String host, int port,
		...
		if (scheme != null && path != null && path.length() > 0
				&& path.charAt(0) != '/') {
			throw new URISyntaxException(path, "Malformed url"); //$NON-NLS-1$
		}
	...
	private class Helper {
		...
		private void parseURI(String uri, boolean forceServer)
			...
			if (index != -1 && (index2 >= index || index2 == -1)
				...
				if (scheme.length() == 0) {
					throw new URISyntaxException(uri, "Malformed URL " + //$NON-NLS-1$
							index);
				}
					...
					if (index != -1) {
						...
						if (authority.length() == 0 && query == null
							...
							throw new URISyntaxException(uri,
									"Malformed URL " + uri.length()); //$NON-NLS-1$
						}
		...
		private void validateScheme(String uri, String scheme, int index)
			...
			if (!((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z'))) {
				throw new URISyntaxException(uri, "" + 0); //$NON-NLS-1$
			}

			try {
				URIEncoderDecoder.validateSimple(scheme, "+-."); //$NON-NLS-1$
			} catch (URISyntaxException e) {
				throw new URISyntaxException(uri, "" + (index //$NON-NLS-1$
						+ e.getIndex()));
			}
		}
		...
		private void validateSsp(String uri, String ssp, int index)
			...
			try {
				...
				throw new URISyntaxException(uri, e //$NON-NLS-1$
		...
		private void parseAuthority(boolean forceServer)
			...
			if (tempHost.equals("")) { //$NON-NLS-1$
				if (forceServer) {
					throw new URISyntaxException(authority, " ", hostindex); //$NON-NLS-1$
				}
				return;
			}
		...
		private void validateUserinfo(String uri, String userinfo, int index)
			...
			for (int i = 0; i < userinfo.length(); i++) {
				...
				if (ch == ']' || ch == '[') {
					throw new URISyntaxException(uri, " ", //$NON-NLS-1$
							index + i);
				}
			}
		}
		...
		private boolean isValidHost(boolean forceServer, String host)
			...
			if (host.charAt(0) == '[') {
				...
				if (host.charAt(host.length() - 1) != ']') {
					throw new URISyntaxException(host, " ", 0); //$NON-NLS-1$
				}
				if (!isValidIP6Address(host)) {
					throw new URISyntaxException(host, " "); //$NON-NLS-1$
				}
				return true;
			}
