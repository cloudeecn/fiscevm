...
public final class Pattern {
	...
	static final int flagsBitMask = Pattern.UNIX_LINES
			| Pattern.CASE_INSENSITIVE | Pattern.COMMENTS | Pattern.MULTILINE
	...
	private Pattern compileImpl(String pattern, int flags)
		...
		if (!lexemes.isEmpty()) {
			throw new PatternSyntaxException("", lexemes.toString(), //$NON-NLS-1$
					lexemes.getIndex());
		}
		finalizeCompile();
		return this;
				...
				if (lexemes.peek() == Lexer.CHAR_AMPERSAND) {
					if (lexemes.lookAhead() == Lexer.CHAR_LEFT_SQUARE_BRACKET) {
						...
						if (firstInClass) {
							...
							// ignore "&&" at "[X&&]" ending where X != empty
							// string
							if (!(lexemes.peek() == Lexer.CHAR_RIGHT_SQUARE_BRACKET)) {
