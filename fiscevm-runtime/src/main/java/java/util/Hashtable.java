...
public class Hashtable<K, V> extends Dictionary<K, V> implements Map<K, V>,
	...
	/**
	 * Removes all key/value pairs from this {@code Hashtable}, leaving the size
	...
	/**
	 * Returns a set of the keys contained in this {@code Hashtable}. The set is
	 * backed by this {@code Hashtable} so changes to one are reflected by the
		...
		for (int i = lastSlot; i >= firstSlot; i--) {
			...
			while (entry != null) {
				if (entry.key != this) {
					...
					// luni.04=this Map
					buffer.append("(this Map)"); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
				}
				buffer.append('=');
				if (entry.value != this) {
