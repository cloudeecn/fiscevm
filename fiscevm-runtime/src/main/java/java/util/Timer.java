...
/**
 * {@code Timer}s are used to schedule jobs for execution in a background
 * process. A single thread is used for the scheduling and this thread has the
 * option of being a daemon thread. By calling {@code cancel} you can terminate
 * a {@code Timer} and its associated thread. All tasks which are scheduled to
 * run after this point are cancelled. Tasks are executed sequentially but are
 * subject to the delays from other tasks run methods. If a specific task takes
 * an excessive amount of time to run it may impact the time at which subsequent
 * tasks may run.
 * <p>
 * 
 * The {@code TimerTask} does not offer any guarantees about the real-time
 * nature of scheduling tasks as its underlying implementation relies on the
 * {@code Object.wait(long)} method.
 * <p>
 * Multiple threads can share a single {@code Timer} without the need for their
 * own synchronization.
 * <p>
 * A {@code Timer} can be set to schedule tasks either at a fixed rate or with a
 * fixed period. Fixed-period execution is the default.
 * <p>
 * The difference between fixed-rate and fixed-period execution is the
 * following: With fixed-rate execution, the start time of each successive run
 * of the task is scheduled in absolute terms without regard for when the
 * previous task run actually took place. This can result in a series of
 * bunched-up runs (one launched immediately after another) if busy resources or
 * other system delays prevent the {@code Timer} from firing for an extended
 * time. With fixed-period execution, each successive run of the task is
 * scheduled relative to the start time of the previous run of the task, so two
 * runs of the task are never fired closer together in time than the specified
 * {@code period}.
 * 
 * @see TimerTask
...
public class Timer {
	...
	private static final class TimerImpl extends Thread {
		...
		/**
		 * Vector consists of scheduled events, sorted according to {@code when}
	 ...
	 * references on the tasks, then after this call they are free to be garbage
	 * collected.
	 * 
	 ...
	 *             if the {@code Timer} has been canceled, or if the task has
	 *             been scheduled or canceled.
	 */
