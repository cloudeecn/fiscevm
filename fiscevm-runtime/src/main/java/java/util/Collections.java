...
public class Collections {
	...
	public static <T> int binarySearch(
		...
		if (!(list instanceof RandomAccess)) {
			ListIterator<? extends Comparable<? super T>> it = list
					.listIterator();
			while (it.hasNext()) {
				int result;
	...
	public static <T> T max(Collection<? extends T> collection,
		...
		if (comparator == null) {
			@SuppressWarnings("unchecked")
			// null comparator? T is comparable
	...
	public static void shuffle(List<?> list, Random random) {
		@SuppressWarnings("unchecked")
		// we won't put foreign objects in
	 ...
	 * {@code newObj}. If the {@code obj} is {@code null}, then all occurrences
	 * of {@code null} are replaced with {@code newObj}.
	 * 
	 * @param list
	 *            the list to modify.
	 * @param obj
	 ...
	 * {@code enumeration}. The elements in the returned {@code ArrayList} are
	 * in the same order as in the {@code enumeration}.
	 * 
	 * @param enumeration
	...
	private static class CheckedListIterator<E> implements ListIterator<E> {
		 ...
		 * 
		 * @param i
		 *            - the listIterator for which a dynamically typesafe view
		 *            to be constructed.
		 */
	...
	private static class CheckedRandomAccessList<E> extends CheckedList<E>
		 ...
		 * 
		 * @param l
		 *            - the randomAccessList for which a dynamically typesafe
		 *            view is to be constructed.
		 */
	...
	private static class CheckedMap<K, V> implements Map<K, V>, Serializable {
		...
		private static class CheckedEntrySet<K, V> implements
			 ...
			 * 
			 * @param s
			 *            - the entry set for which a dynamically typesafe view
	...
	private static class CheckedSortedSet<E> extends CheckedSet<E> implements
		 ...
		 * 
		 * @param s
		 *            - the sortedSet for which a dynamically typesafe view is
		 *            to be constructed.
		 */
	...
	private static class CheckedSortedMap<K, V> extends CheckedMap<K, V>
		 ...
		 * 
		 * @param m
		 *            - the sortedMap for which a dynamically typesafe view is
		 *            to be constructed.
		 */
