...
class Lexer {
	...
	static String normalize(String input) {
		...
		/*
		 * Canonical ordering. See
		 * http://www.unicode.org/reports/tr15/#Decomposition for details
		 */
		resCodePoints = Lexer.getCanonicalOrder(resCodePoints,
				resCodePointsIndex);

		/*
		 * Decomposition for Hangul syllables. See
		 * http://www.unicode.org/reports/tr15/#Hangul for details
		 */
		...
		for (int i = 0; i < resCodePointsIndex; i++) {
			...
			if (decomp == null) {
				...
				/*
				 * Note that Hangul decompositions have length that is equal 2
				 * or 3.
				 */
	...
	private void movePointer() {
		...
		do {
			...
			if (lookAhead == '\\') {
				...
				switch (lookAhead) {
				case -1:
					throw new PatternSyntaxException("", "", 1); //$NON-NLS-1$
				case 'P':
				case 'p': {
					...
					try {
						...
						throw new PatternSyntaxException("" //$NON-NLS-1$
								, this.toString(), index);
					}
					lookAhead = 0;
					break;
						...
						do {
							if (!behind) {
								...
								default: {
									 ...
									 * readFlags() if we read (?idmsux-idmsux)
									 */
									if (lookAhead >= 256) {
	...
	private String parseCharClassName() {
		...
		if (index < pattern.length - 2) {
			...
			if (ch != '}')
				throw new PatternSyntaxException("", this //$NON-NLS-1$
						.toString(), index);
		}

		if (sb.length() == 0)
			throw new PatternSyntaxException("", this.toString(), //$NON-NLS-1$
					index);
