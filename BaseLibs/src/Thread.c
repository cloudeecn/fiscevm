...
static void processThrowable(fy_VMContext *context, fy_thread *thread,
		jint handle, fy_exception *exception) {
...
void fy_threadPutLocalType(fy_VMContext *context, fy_thread *thread, int pos,
	...
	ASSERT(type!=TH_TYPE_HANDLE||(value>=0&&value<MAX_OBJECTS));
...
jint fy_threadGetLocalHandle(fy_VMContext *context, fy_thread *thread, int pos) {
	int framepos = getLB(context, thread) + pos;
	ASSERT(pos<getLC(context,thread)&&pos>=0);
	ASSERT(iframe(thread,getLTB(context, thread) + pos)==TH_TYPE_HANDLE);
	return iframe(thread,framepos);
}
...
jint fy_threadGetLocalInt(fy_VMContext *context, fy_thread *thread, int pos) {
	int framepos = getLB(context, thread) + pos;
	ASSERT(pos<getLC(context,thread)&&pos>=0);
	ASSERT(iframe(thread,getLTB(context, thread) + pos)==TH_TYPE_INT);
	return iframe(thread,framepos);
}
...
jlong fy_threadGetLocalLong(fy_VMContext *context, fy_thread *thread, int pos) {
	int framepos = getLB(context, thread) + pos;
	ASSERT((pos<getLC(context,thread))&&(pos>=0));
	ASSERT(iframe(thread,getLTB(context, thread) + pos)==TH_TYPE_WIDE);
...
jint fy_threadGetLocalReturn(fy_VMContext *context, fy_thread *thread, int pos) {
	int framepos = getLB(context, thread) + pos;
	ASSERT(pos<getLC(context,thread)&&pos>=0);
	ASSERT(iframe(thread,getLTB(context, thread) + pos)==TH_TYPE_RETURN);
	return iframe(thread,framepos);
}
...
void fy_threadPushHandle(fy_VMContext *context, fy_thread *thread, jint value) {
	ASSERT(getSR(context,thread)<getSC(context,thread));
	ASSERT(value>=0||value<MAX_OBJECTS);
	iframe(thread,getSB(context,thread)+getSR(context,thread)) = value;
...
jlong fy_threadPopLong(fy_VMContext *context, fy_thread *thread) {
	...
	ASSERT(getSR(context,thread)>0);
	ASSERT(
			iframe(thread,getSTB(context,thread)+getSR(context,thread)-2)==TH_TYPE_WIDE);
	ASSERT(
...
fy_message fy_threadRun(fy_VMContext *context, fy_thread *thread, jint ops) {
	...
	for (opCount = 0; opCount < ops; opCount++) {
		...
		if (thread->currentThrowable) {
			processThrowable(context, thread, thread->currentThrowable,
					exception);
			if (exception->exceptionType != exception_none) {
		...
		case AASTORE: {
			...
			if (hvalue != 0
					&& !fy_classCanCastTo(context,
							fy_heapGetClassOfObject(context, hvalue), clazz2)) {
				exception->exceptionType = exception_normal;
		...
		case ANEWARRAY: {
			...
			fy_threadPushHandle(context, thread,
					fy_heapAllocateArray(context, clazz2, index, exception));
			if (exception->exceptionType != exception_none) {
				message.messageType = message_exception;
				break;/*EXCEPTION_THROWN*/
			}
		...
		case DALOAD: {
			index = fy_threadPopInt(context, thread);
			aref = fy_threadPopHandle(context, thread);
			dvalue1 = fy_heapGetArrayDouble(context, aref, index, exception);
			if (exception->exceptionType != exception_none) {
				message.messageType = message_exception;
				break;
			}
		...
		case FREM: {
			...
			if (fvalue2 == 0) {
				...
				fy_threadPushFloat(context, thread,
						fvalue1 - (float) floor(fvalue1 / fvalue2) * fvalue2);
			}
			break;
		}
		case FSUB: {
		...
		case GETFIELD: {
			...
			if (field->access_flags & fy_ACC_STATIC) {
				...
				sprintf_s(exception->exceptionDesc, sizeof(exception->exceptionDesc), "field %s is static",
						msg);
				break;
			}
			...
			case 'D':
			case 'J':
				lvalue = fy_heapGetFieldLong(context, aref, field, exception);
				if (exception->exceptionType != exception_none) {
					message.messageType = message_exception;
					break;
				}
			...
			case 'L':
			case '[':
				aref = fy_heapGetFieldHandle(context, aref, field, exception);
				if (exception->exceptionType != exception_none) {
					message.messageType = message_exception;
					break;
				}
				...
				break;
			default:
				ivalue = fy_heapGetFieldInt(context, aref, field, exception);
				if (exception->exceptionType != exception_none) {
					message.messageType = message_exception;
					break;
				}
		...
		case IDIV: {
			...
			if (ivalue == 0) {
				...
				strcpy_s(exception->exceptionDesc,
						sizeof(exception->exceptionDesc), "Divided by zero!");
			}
		...
		case IINC: {
			...
			ivalue = nextS1(context, thread);
			fy_threadPutLocalInt(context, thread, index,
					fy_threadGetLocalInt(context, thread, index) + ivalue);
			break;
		}
		case IMUL: {
			fy_threadPushInt(
		...
		case INVOKEINTERFACE: {
			...
			if (mvalue->access_flags & fy_ACC_NATIVE) {
				...
				if (nh == NULL) {
					...
					(nh->handler)(context, thread, nh->data, pivalue, ivalue,
							exception);
					if (exception->exceptionType != exception_none) {
						message.messageType = message_exception;
						vm_free(pivalue);
		...
		case INVOKESPECIAL: {
			...
			if (fy_strCmp(mvalue->name, context->sInit) > 0
				...
				fy_strSPrint(exception->exceptionDesc,
						sizeof(exception->exceptionDesc), mvalue->uniqueName);
				vm_free(pivalue);
				vm_free(pbvalue);
				break;
			}
		...
		case NEW: {
			...
			if (clazz1->accessFlags & (fy_ACC_INTERFACE | fy_ACC_ABSTRACT)) {
		...
		case PUTFIELD: {
			...
			if (field->access_flags & fy_ACC_STATIC) {
				...
				fy_strSPrint(msg, 256, field->uniqueName);
				sprintf_s(exception->exceptionDesc,
						sizeof(exception->exceptionDesc), "Field %s is static",
						msg);
				break;
			}
			...
			if ((field->access_flags & fy_ACC_FINAL)
				...
				fy_strSPrint(msg, 256, field->uniqueName);
				sprintf_s(exception->exceptionDesc,
						sizeof(exception->exceptionDesc), "Field %s is final",
						msg);
				break;
			}
			...
			case 'D':
			case 'J':
				fy_heapPutFieldLong(context, aref, field, lvalue, exception);
				break;
			default:
		...
		if (message.messageType == message_exception) {
			...
			/*
			 vm_die("Exception caught %s: %s", exception->exceptionName,
