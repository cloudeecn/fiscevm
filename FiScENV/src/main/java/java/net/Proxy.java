 ...
 * {@code Proxy} stores a type and an address and is immutable. There are three
 * types of proxies:
 * <ul>
 * <li>DIRECT</li>
 * <li>HTTP</li>
...
public class Proxy {
	 ...
	 *             if the parameter {@code type} is set to
	 *             {@code Proxy.Type.DIRECT} or the value for
	...
	public Proxy(Proxy.Type type, SocketAddress sa) {
		...
		if (type == Type.DIRECT || null == sa) {
			...
			throw new IllegalArgumentException(
					"Illegal Proxy.Type or SocketAddress argument"); //$NON-NLS-1$
		}
		this.type = type;
		address = sa;
	}
	 ...
	 * includes the two parts {@code type.toString()} and
	 * {@code address.toString()} if {@code address} is not {@code null}.
	 *
	 ...
	 *         {@code Proxy} as this instance, {@code false} otherwise.
	 * @see #hashCode
	 */
	@Override
	...
	/**
	 * {@code Enum} class for the proxy type. Possible options are
	 * {@code DIRECT}, {@code HTTP} and {@code SOCKS}.
	 */
	public enum Type {
		/**
