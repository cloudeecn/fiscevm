...
public final class URL implements java.io.Serializable {
	...
	public static synchronized void setURLStreamHandlerFactory(
		...
		if (streamHandlerFactory != null) {
			throw new Error("URLStreamHandlerFactory already setted."); //$NON-NLS-1$
		}
	...
	public URL(URL context, String spec, URLStreamHandler handler)
		...
		if (protocol != null) {
			...
			if (context == null) {
				throw new MalformedURLException("context is null"); //$NON-NLS-1$
			}
			set(context.getProtocol(), context.getHost(), context.getPort(),
					context.getAuthority(), context.getUserInfo(),
					context.getPath(), context.getQuery(), null);
			if (strmHandler == null) {
				strmHandler = context.strmHandler;
			}
		}
		...
		if (strmHandler == null) {
			...
			if (strmHandler == null) {
				throw new MalformedURLException(
						"Unsupported protocal " + protocol); //$NON-NLS-1$
			}
		}
		...
		if (port < -1) {
			throw new MalformedURLException("Illegal port " + port); //$NON-NLS-1$
		}
	}
	...
	public URL(String protocol, String host, int port, String file,
		...
		if (protocol == null) {
			throw new NullPointerException("Protocal is null"); //$NON-NLS-1$ //$NON-NLS-2$
		}
		...
		if (handler == null) {
			setupStreamHandler();
			if (strmHandler == null) {
				throw new MalformedURLException("streamHandler is null"); //$NON-NLS-1$
			}
		} else {
			strmHandler = handler;
		}
	 ...
	 * @return {@code true} if both instances represents the same URL,
	...
	public URLConnection openConnection(Proxy proxy) throws IOException {
		if (proxy == null) {
			...
			throw new IllegalArgumentException("proxy should not be null"); //$NON-NLS-1$
		}
