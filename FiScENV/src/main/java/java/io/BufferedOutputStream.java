 ...
 *
 * <pre>
 * BufferedOutputStream buf = new BufferedOutputStream(new FileOutputStream(
 * 		&quot;file.java&quot;));
 * </pre>
 *
	...
	public synchronized void write(byte[] buffer, int offset, int length)
		...
		if (offset < 0 || offset > buffer.length - length) {
			...
			throw new ArrayIndexOutOfBoundsException(
					"Offset ou of bounds " + offset); //$NON-NLS-1$

		}
		if (length < 0) {
			// luni.18=Length out of bounds \: {0}
			throw new ArrayIndexOutOfBoundsException(
					"Length out of bounds : " + length); //$NON-NLS-1$
		}
		...
		// the length is always less than (internalBuffer.length - count) here
		// so arraycopy is safe
