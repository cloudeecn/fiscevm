...
public class LineNumberInputStream extends FilterInputStream {
	 ...
	 * line number count will also be reset to the last marked line number
	 * count.
	 * <p>
	 ...
	 * them in the byte array {@code buffer} starting at {@code offset}. Returns
	 * the number of bytes actually read or -1 if no bytes have been read and
	 * the end of this stream has been reached.
	 * <p>
	 * The line number count is incremented if a line terminator is encountered.
	 * Recognized line terminator sequences are {@code '\r'}, {@code '\n'} and
	 * {@code "\r\n"}. Line terminator sequences are always translated into
	...
	public int read(byte[] buffer, int offset, int length) throws IOException {
		...
		if (offset > buffer.length || offset < 0) {
			...
			throw new ArrayIndexOutOfBoundsException("" + offset); //$NON-NLS-1$
		}
		if (length < 0 || length > buffer.length - offset) {
			// luni.18=Length out of bounds \: {0}
			throw new ArrayIndexOutOfBoundsException("" + length); //$NON-NLS-1$
		}
	...
	/**
	 * Sets the line number of this stream to the specified {@code lineNumber}.
