 ...
 * {@link Pattern} instance and an input text. A typical use case is to
...
public final class Matcher implements MatchResult {
	...
	private String processReplacement(String replacement) {
		if (this.replacement != null && this.replacement.equals(replacement)) {
			...
			while (index < repl.length) {
				...
				if (nextBackSlashed) {
					...
					if (repl[index] == '$') {
						...
						try {
							final int gr = Integer.parseInt(new String(repl,
									++index, 1));
			...
			if (replacementParts != null && replacementPos != res.length()) {
				replacementParts.add(res.subSequence(replacementPos,
						res.length()));
			}
			return res.toString();
		}
	...
	public Matcher region(int start, int end) {
		...
		if (start > end || start < 0 || end < 0 || start > string.length()
				|| end > string.length()) {
