...
public class TreeMap<K, V> extends AbstractMap<K, V> implements
	...
	private static abstract class AbstractSubMapIterator<K, V> {
			...
			if (expectedModCount == subMap.m.modCount) {
				if (expectedModCount == subMap.m.modCount) {
					...
					if (lastNode != null) {
						...
						if (node != null
								&& !this.subMap.isInRange(node.keys[offset])) {
							node = null;
						}
						lastNode = null;
						expectedModCount++;
		...
		Entry<K, V> createEntry(Node<K, V> node, int index) {
			TreeMap.Entry<K, V> entry = new TreeMap.Entry<K, V>(
	...
	private abstract static class AscendingSubMapIterator<K, V> extends
		...
		TreeMap.Entry<K, V> getNext() {
			...
			if (lastNode != null) {
				boundaryPair = getBoundaryNode();
				if (boundaryPair != null && boundaryPair.node == lastNode
						&& boundaryPair.index == lastOffset) {
					node = null;
				}
	...
	private abstract static class DescendingSubMapIterator<K, V> extends
		...
		DescendingSubMapIterator(NavigableSubMap<K, V> map) {
			...
			if (map.fromStart) {
				entry = map.loInclusive ? map.m.findFloorEntry(map.lo) : map.m
						.findLowerEntry(map.lo);
			} else {
		...
		final TreeMap.Entry<K, V> getBoundaryNode() {
			if (subMap.toEnd) {
				return subMap.hiInclusive ? subMap.m
						.findCeilingEntry(subMap.hi) : subMap.m
						.findHigherEntry(subMap.hi);
			}
	...
	static final class SubMap<K, V> extends AbstractMap<K, V> implements
		...
		SubMap(K start, boolean hasStart, TreeMap<K, V> map, K end,
				boolean hasEnd) {
			backingMap = map;
		...
		private void setFirstKey() {
			...
			if (hasEnd && foundNode != null) {
				Comparator<? super K> cmp = backingMap.comparator;
				if (cmp == null) {
					isBounded = (toComparable(foundNode.keys[foundIndex])
							.compareTo(endKey) <= 0);
				} else {
					isBounded = (cmp
							.compare(foundNode.keys[foundIndex], endKey) <= 0);
				}
			}
		...
		public SortedMap<K, V> headMap(K endKey) {
			...
			if (cmp == null) {
				...
				if (hasEnd
						&& backingMap.comparator().compare(endKey, this.endKey) >= 0) {
					throw new IllegalArgumentException();
				}
			}
			if (hasStart) {
		...
		private void setLastKey() {
			...
			TOP_LOOP: while (node != null) {
				...
				int result = object != null ? object.compareTo(keys[left_idx])
						: -backingMap.comparator.compare(keys[left_idx], key);
			...
			if (backingMap.size > 0 && !(startKey.equals(endKey))) {
				if (!hasEnd) {
					...
					if (lastKeyNode != null) {
						Comparable<K> object = backingMap.comparator == null ? toComparable((K) endKey)
								: null;
						if (backingMap.cmp(object, endKey,
								lastKeyNode.keys[lastKeyIndex]) != 0) {
							...
							if (lastKeyIndex != lastKeyNode.left_idx) {
								...
								// startkey, there's no lastkey
								if (backingMap.cmp(object, startKey,
										lastKeyNode.keys[lastKeyIndex - 1]) <= 0) {
		...
		public SortedMap<K, V> subMap(K startKey, K endKey) {
			...
			if (cmp == null) {
				...
				if (hasStart
						&& backingMap.comparator().compare(endKey,
								this.startKey) < 0) {
					throw new IllegalArgumentException();
				}
				if (hasEnd
		...
		public Iterator<V> iterator() {
			...
			int toIndex = subMap.lastKeyIndex
					+ (subMap.lastKeyNode != null
							&& (!subMap.lastKeyNode.keys[subMap.lastKeyIndex]
									.equals(subMap.endKey)) ? 1 : 0);
			if (to != null && toIndex > to.right_idx) {
				to = to.next;
				toIndex = to != null ? to.left_idx : 0;
				if (to == null) {
					// has endkey but it does not exist, thus return
			...
			return new BoundedValueIterator<K, V>(from, from == null ? 0
					: fromIndex, subMap.backingMap, to, to == null ? 0
					: toIndex);
		}
	...
	static class BoundedMapIterator<K, V> extends AbstractMapIterator<K, V> {
		...
		void makeBoundedNext() {
			if (node != null) {
				boolean endOfIterator = lastNode == finalNode
	...
	static class SubMapKeySet<K, V> extends AbstractSet<K> implements Set<K> {
		...
		public Iterator<K> iterator() {
			...
			if (from == null) {
				return new BoundedKeyIterator<K, V>(null, 0, subMap.backingMap,
						null, 0);
			}
			if (!subMap.hasEnd) {
	...
	static class SubMapEntrySet<K, V> extends AbstractSet<Map.Entry<K, V>>
		...
		public Iterator<Map.Entry<K, V>> iterator() {
			...
			if (from == null) {
				return new BoundedEntryIterator<K, V>(null, 0,
						subMap.backingMap, null, 0);
			}
			if (!subMap.hasEnd) {
		...
		public boolean contains(Object object) {
			if (object instanceof Map.Entry) {
				...
				if (subMap.isInRange(key)) {
					...
					return v1 == null ? (v2 == null && subMap.containsKey(key))
							: v1.equals(v2);
				}
			}
			return false;
	...
	static class AscendingSubMapEntrySet<K, V> extends
			AbstractSet<Map.Entry<K, V>> implements
		...
		AscendingSubMapEntrySet(NavigableSubMap<K, V> map,
			...
			if (startEntry != null && endEntry != null) {
				this.map = (NavigableSubMap<K, V>) map.subMap(
						startEntry.getKey(), startInclusive, endEntry.getKey(),
						endInclusive);
				return;
			}
			if (startEntry != null) {
				this.map = (NavigableSubMap<K, V>) map.tailMap(
						startEntry.getKey(), startInclusive);
				return;
			}
			if (endEntry != null) {
				this.map = (NavigableSubMap<K, V>) map.headMap(
						endEntry.getKey(), endInclusive);
				return;
			}
			this.map = map;
		}
		...
		public NavigableSet<java.util.Map.Entry<K, V>> headSet(
			...
			if (isInRange) {
				return new AscendingSubMapEntrySet<K, V>(map, null, false, end,
						endInclusive);
			}
			throw new IllegalArgumentException();
		}
		...
		public java.util.Map.Entry<K, V> higher(java.util.Map.Entry<K, V> e) {
			...
			if (cmp == null) {
				...
				if (hasStart
						&& cmp.compare(e.getKey(), startEntry.getKey()) < 0) {
					return map.higherEntry(startEntry.getKey());
				}
		...
		public NavigableSet<java.util.Map.Entry<K, V>> subSet(
			...
			if (map.fromStart
					&& ((!map.loInclusive && endInclusive) ? map.m.keyCompare(
							end.getKey(), map.lo) <= 0 : map.m.keyCompare(
							end.getKey(), map.lo) < 0)) {
				throw new IllegalArgumentException();
			}
			if (map.toEnd
					&& ((!map.hiInclusive && startInclusive) ? map.m
							.keyCompare(start.getKey(), map.hi) >= 0 : map.m
							.keyCompare(start.getKey(), map.hi) > 0)) {
				throw new IllegalArgumentException();
			}
			return new AscendingSubMapEntrySet<K, V>(map, start,
		...
		public NavigableSet<java.util.Map.Entry<K, V>> tailSet(
			...
			if (map.toEnd) {
				result = (null != comparator()) ? comparator().compare(
						start.getKey(), map.hi) : toComparable(start.getKey())
						.compareTo(map.hi);
			...
			if (map.fromStart) {
				result = (null != comparator()) ? comparator().compare(
						start.getKey(), map.lo) : toComparable(start.getKey())
						.compareTo(map.lo);
				isInRange = isInRange
		...
		public SortedSet<java.util.Map.Entry<K, V>> headSet(
				java.util.Map.Entry<K, V> end) {
		...
		public SortedSet<java.util.Map.Entry<K, V>> subSet(
			...
			if ((null != comparator()) ? comparator().compare(start.getKey(),
					end.getKey()) > 0 : toComparable(start.getKey()).compareTo(
					end.getKey()) > 0) {
				throw new IllegalArgumentException();
			}
			...
			return new AscendingSubMapEntrySet<K, V>(map, start, false, end,
					false);
		}

		public SortedSet<java.util.Map.Entry<K, V>> tailSet(
				java.util.Map.Entry<K, V> start) {
	...
	static class DescendingSubMapEntrySet<K, V> extends
		...
		public java.util.Map.Entry<K, V> ceiling(java.util.Map.Entry<K, V> e) {
			...
			if (!map.checkLowerBound(node.key)) {
				Comparable<K> object = map.comparator() == null ? toComparable((K) e
						.getKey()) : null;
				TreeMap.Entry<K, V> first = map.loInclusive ? map.m
						.findFloorEntry(map.lo) : map.m.findLowerEntry(map.lo);
				if (first != null
						&& map.cmp(object, e.getKey(), first.getKey()) <= 0) {
					node = first;
				} else {
					node = null;
				}
		...
		public java.util.Map.Entry<K, V> floor(java.util.Map.Entry<K, V> e) {
			...
			if (!map.checkLowerBound(node.key)) {
				Comparable<K> object = map.m.comparator == null ? toComparable((K) e
						.getKey()) : null;
				TreeMap.Entry<K, V> first = map.hiInclusive ? map.m
						.findCeilingEntry(map.hi) : map.m
						.findHigherEntry(map.hi);
				if (first != null
						&& map.cmp(object, e.getKey(), first.getKey()) < 0) {
					node = first;
				} else {
		...
		void checkInRange(K key, boolean keyInclusive) {
			...
			int result = 0;
			if (map.toEnd) {
				result = (null != map.comparator()) ? comparator().compare(key,
						map.hi) : toComparable(key).compareTo(map.hi);
		...
		public NavigableSet<java.util.Map.Entry<K, V>> headSet(
			...
			if (map.toEnd) {
				result = (null != map.comparator()) ? comparator().compare(
						end.getKey(), map.hi) : toComparable(end.getKey())
						.compareTo(map.hi);
			...
			if (map.fromStart) {
				result = (null != comparator()) ? comparator().compare(
						end.getKey(), map.lo) : toComparable(end.getKey())
						.compareTo(map.lo);
			...
			if (map.fromStart) {
				...
				return new DescendingSubMapEntrySet<K, V>(
						new DescendingSubMap<K, V>(map.m, end.getKey(),
								endInclusive));
			}
		}
		...
		public java.util.Map.Entry<K, V> higher(java.util.Map.Entry<K, V> e) {
			...
			if (node != null && !map.checkUpperBound(node.key)) {
				node = map.hiInclusive ? map.findFloorEntryImpl(map.hi) : map
						.findLowerEntryImpl(map.hi);
			}
			Comparable<K> object = map.comparator() == null ? toComparable((K) e
					.getKey()) : null;
		...
		public java.util.Map.Entry<K, V> lower(java.util.Map.Entry<K, V> e) {
			...
			if (node != null && !map.checkUpperBound(node.key)) {
				node = map.loInclusive ? map.findCeilingEntryImpl(map.hi) : map
						.findHigherEntryImpl(map.hi);
			}
			Comparable<K> object = map.m.comparator == null ? toComparable((K) e
					.getKey()) : null;
			if (node != null && (map.cmp(object, e.getKey(), node.key)) >= 0) {
	...
	static class AscendingSubMapKeySet<K, V> extends AbstractSet<K> implements
			NavigableSet<K> {
		NavigableSubMap<K, V> map;
		...
		public NavigableSet<K> subSet(K start, boolean startInclusive, K end,
			...
			if (map.fromStart
					&& ((!map.loInclusive && startInclusive) ? map.m
							.keyCompare(start, map.lo) <= 0 : map.m.keyCompare(
							start, map.lo) < 0)
					|| (map.toEnd && ((!map.hiInclusive && (endInclusive || (startInclusive && start
							.equals(end)))) ? map.m.keyCompare(end, map.hi) >= 0
			...
			return new AscendingSubMapKeySet<K, V>(new AscendingSubMap<K, V>(
	...
	static class DescendingSubMapKeySet<K, V> extends AbstractSet<K> implements
			NavigableSet<K> {
		NavigableSubMap<K, V> map;
		...
		public K ceiling(K e) {
			...
			if (node != null && !map.checkLowerBound(node.key)) {
				Entry<K, V> first = map.loInclusive ? map.m
						.findFloorEntry(map.lo) : map.m.findLowerEntry(map.lo);
				if (first != null && map.cmp(object, e, first.key) <= 0
						&& map.checkUpperBound(first.key)) {
					node = first;
				} else {
					node = null;
				}
		...
		public K floor(K e) {
			...
			if (node != null && !map.checkUpperBound(node.key)) {
				node = map.hiInclusive ? map.m.findCeilingEntry(map.hi) : map.m
						.findHigherEntry(map.hi);
			}
		...
		public NavigableSet<K> subSet(K start, boolean startInclusive, K end,
			...
			checkInRange(end, endInclusive);
			if ((null != map.comparator()) ? map.comparator().compare(start,
					end) > 0 : toComparable(start).compareTo(end) > 0) {
				throw new IllegalArgumentException();
			}
	...
	static abstract class NavigableSubMap<K, V> extends AbstractMap<K, V>
		...
		public SortedMap<K, V> headMap(K end) {
			if (toEnd) {
				int result = (null != comparator()) ? comparator().compare(end,
						hi) : toComparable(end).compareTo(hi);
				if (result > 0) {
					throw new IllegalArgumentException();
				}
			}
		...
		public SortedMap<K, V> tailMap(K start) {
			...
			if (toEnd) {
				int result = (null != comparator()) ? comparator().compare(
						start, hi) : toComparable(start).compareTo(hi);
		...
		final boolean checkUpperBound(K key) {
			if (toEnd) {
				int result = (null != comparator()) ? comparator().compare(key,
						hi) : toComparable(key).compareTo(hi);
		...
		final boolean checkLowerBound(K key) {
			if (fromStart) {
				int result = -((null != comparator()) ? comparator().compare(
						lo, key) : toComparable(lo).compareTo(key));
		...
		final TreeMap.Entry<K, V> theSmallestEntry() {
			...
			if (!fromStart) {
				...
				result = loInclusive ? m.findCeilingEntry(lo) : m
						.findHigherEntry(lo);
			}
			return (null != result && (!toEnd || checkUpperBound(result
					.getKey()))) ? result : null;
		}
		...
		final TreeMap.Entry<K, V> theBiggestEntry() {
			...
			if (!toEnd) {
				...
				result = hiInclusive ? m.findFloorEntry(hi) : m
						.findLowerEntry(hi);
			}
			return (null != result && (!fromStart || checkLowerBound(result
					.getKey()))) ? result : null;
		}
		...
		private TreeMap.Entry<K, V> findFloorEntry(K key) {
			...
			if (node != null && fromStart && !checkLowerBound(node.key)) {
				Comparable<K> object = m.comparator == null ? toComparable((K) key)
						: null;
		...
		private int cmp(Comparable<K> object, K key1, K key2) {
			return object != null ? object.compareTo(key2) : comparator()
					.compare(key1, key2);
		}
		...
		public Collection<V> values() {
			if (valuesCollection == null) {
				if (!this.toEnd && !this.fromStart) {
					...
					if (loInclusive) {
						startEntry = fromStart ? m.ceilingEntry(this.lo)
								: theSmallestEntry();
					} else {
						startEntry = fromStart ? m.findHigherEntry(this.lo)
								: theSmallestEntry();
					}
					if (startEntry == null) {
					...
					valuesCollection = new SubMapValuesCollection<K, V>(
							new SubMap<K, V>(startK, true, this.m, lastK,
									lastK == null ? false : toEnd));
				}
			}
			return valuesCollection;
		}
	...
	static class NullSubMapValuesCollection<K, V> extends
			SubMapValuesCollection<K, V> {
		SubMap<K, V> subMap;
	...
	static class AscendingSubMap<K, V> extends NavigableSubMap<K, V> implements
		...
		public NavigableMap<K, V> descendingMap() {
			if (fromStart && toEnd) {
				return new DescendingSubMap<K, V>(hi, hiInclusive, m, lo,
						loInclusive);
			}
			if (fromStart) {
		...
		public NavigableMap<K, V> subMap(K start, boolean startKeyInclusive,
				K end, boolean endKeyInclusive) {
			if (fromStart
					&& ((!loInclusive && startKeyInclusive) ? m.keyCompare(
							start, lo) <= 0 : m.keyCompare(start, lo) < 0)
					|| (toEnd && ((!hiInclusive && (endKeyInclusive || (startKeyInclusive && start
							.equals(end)))) ? m.keyCompare(end, hi) >= 0 : m
							.keyCompare(end, hi) > 0))) {
				throw new IllegalArgumentException();
			}
		...
		public NavigableMap<K, V> headMap(K end, boolean inclusive) {
			if (fromStart
					&& ((!loInclusive && inclusive) ? m.keyCompare(end, lo) <= 0
							: m.keyCompare(end, lo) < 0)) {
				throw new IllegalArgumentException();
			}
			if (toEnd
					&& ((!hiInclusive && inclusive) ? m.keyCompare(end, hi) >= 0
							: m.keyCompare(end, hi) > 0)) {
				throw new IllegalArgumentException();
			}
			if (checkUpperBound(end)) {
				if (this.fromStart) {
					return new AscendingSubMap<K, V>(this.lo, this.loInclusive,
							m, end, inclusive);
				}
		...
		public NavigableMap<K, V> tailMap(K start, boolean inclusive) {
			if (fromStart
					&& ((!loInclusive && inclusive) ? m.keyCompare(start, lo) <= 0
							: m.keyCompare(start, lo) < 0)) {
				throw new IllegalArgumentException();
			}
			if (toEnd
					&& ((!hiInclusive && inclusive) ? m.keyCompare(start, hi) >= 0
							: m.keyCompare(start, hi) > 0)) {
				throw new IllegalArgumentException();
			}
			if (checkLowerBound(start)) {
				if (this.toEnd) {
					return new AscendingSubMap<K, V>(start, inclusive, m,
							this.hi, this.hiInclusive);
				}
				return new AscendingSubMap<K, V>(start, inclusive, m);
			} else {
				return this;
	...
	static class DescendingSubMap<K, V> extends NavigableSubMap<K, V> implements
		...
		public Map.Entry<K, V> firstEntry() {
			...
			if (!fromStart) {
				...
				result = loInclusive ? m.findFloorEntry(lo) : m
						.findLowerEntry(lo);
			}
		...
		public Map.Entry<K, V> pollFirstEntry() {
			...
			if (node != null
					&& fromStart
					&& (loInclusive ? this.m.keyCompare(lo, node.key) < 0
							: this.m.keyCompare(lo, node.key) <= 0)) {
				node = null;
			}
			if (node != null
					&& toEnd
					&& (hiInclusive ? this.m.keyCompare(hi, node.key) > 0
							: this.m.keyCompare(hi, node.key) >= 0)) {
				node = null;
			}
			SimpleImmutableEntry<K, V> result = m.newImmutableEntry(node);
			if (null != node) {
		...
		public Map.Entry<K, V> pollLastEntry() {
			TreeMap.Entry<K, V> node = null;
			if (toEnd) {
				node = hiInclusive ? this.m.findCeilingEntry(hi) : this.m
						.findHigherEntry(hi);
			} else {
		...
		public Map.Entry<K, V> higherEntry(K key) {
			...
			if (null != entry
					&& (fromStart && !checkLowerBound(entry.getKey()))) {
				entry = loInclusive ? this.m.findFloorEntry(this.lo) : this.m
						.findLowerEntry(this.lo);
			}
		...
		public Map.Entry<K, V> lowerEntry(K key) {
			...
			if (null != entry && (toEnd && !checkUpperBound(entry.getKey()))) {
				entry = hiInclusive ? this.m.findCeilingEntry(this.hi) : this.m
						.findHigherEntry(this.hi);
			}
			if (null != entry && (!isInRange(entry.getKey()))) {
				entry = null;
			}
		...
		public Map.Entry<K, V> ceilingEntry(K key) {
			...
			if (toEnd && m.cmp(object, key, lo) >= 0) {
				entry = loInclusive ? this.m.findFloorEntry(lo) : this.m
						.findLowerEntry(lo);
			} else {
		...
		public Map.Entry<K, V> floorEntry(K key) {
			...
			if (fromStart && m.cmp(object, key, hi) <= 0) {
				entry = hiInclusive ? this.m.findCeilingEntry(hi) : this.m
						.findHigherEntry(hi);
			} else {
		...
		public NavigableMap<K, V> descendingMap() {
			if (fromStart && toEnd) {
				return new AscendingSubMap<K, V>(hi, hiInclusive, m, lo,
						loInclusive);
			}
			if (fromStart) {
		...
		public NavigableMap<K, V> subMap(K start, boolean startKeyInclusive,
			...
			if (fromStart
					&& ((!loInclusive && (startKeyInclusive || (endKeyInclusive && start
							.equals(end)))) ? keyCompare(start, lo) <= 0
							: keyCompare(start, lo) < 0)
					|| (toEnd && ((!hiInclusive && (endKeyInclusive)) ? keyCompare(
							end, hi) >= 0 : keyCompare(end, hi) > 0))) {
				throw new IllegalArgumentException();
			}
			...
			if (null != inclusiveEnd) {
				...
				if (toEnd) {
					result = (null != comparator()) ? comparator().compare(
							...
							.compareTo(hi);
					isInRange = (hiInclusive || !inclusive) ? result <= 0
							: result < 0;
				}
				if (fromStart) {
					result = (null != comparator()) ? comparator().compare(
							...
							.compareTo(lo);
					isInRange = isInRange
							&& ((loInclusive || !inclusive) ? result >= 0
									: result > 0);
				}
			}
			if (isInRange) {
				if (this.fromStart) {
					return new DescendingSubMap<K, V>(this.lo,
		...
		public Collection<V> values() {
			if (valuesCollection == null) {
				if (fromStart || toEnd) {
					return valuesCollection = new DescendingSubMapValuesCollection<K, V>(
							this);
				}
				valuesCollection = super.values();
			}
		...
		static class DescendingSubMapValuesCollection<K, V> extends
				AbstractCollection<V> {
			DescendingSubMap<K, V> subMap;

			public DescendingSubMapValuesCollection(
					DescendingSubMap<K, V> subMap) {
				this.subMap = subMap;
			}
			...
			public Iterator<V> iterator() {
				...
				TreeMap.Entry<K, V> to = subMap.m.findLowerEntry(subMap
						.lastKey());
				return new DescendingValueIterator<K, V>(from.node,
						from == null ? 0 : from.index, subMap,
						to == null ? null : to.node, to == null ? 0 : to.index);
			}

			static class DescendingValueIterator<K, V> extends
					BoundedMapIterator<K, V> implements Iterator<V> {

				public DescendingValueIterator(Node<K, V> startNode,
						int startOffset, DescendingSubMap<K, V> map,
						Node<K, V> finalNode, int finalOffset) {
	...
	Entry<K, V> find(Object keyObj) {
		Comparable<K> object = comparator == null ? toComparable((K) keyObj)
				: null;
		K keyK = (K) keyObj;
		Node<K, V> node = root;
		while (node != null) {
	...
	Entry<K, V> createEntry(Node<K, V> node, int index) {
		TreeMap.Entry<K, V> entry = new TreeMap.Entry<K, V>(node.keys[index],
				node.values[index]);
		entry.node = node;
		entry.index = index;
		return entry;
	...
	TreeMap.Entry<K, V> findSmallestEntry() {
		if (null != root) {
			Node<K, V> node = minimum(root);
			TreeMap.Entry<K, V> ret = new TreeMap.Entry<K, V>(
					node.keys[node.left_idx], node.values[node.left_idx]);
			ret.node = node;
	...
	TreeMap.Entry<K, V> findBiggestEntry() {
		if (null != root) {
			...
			TreeMap.Entry<K, V> ret = new TreeMap.Entry<K, V>(
					node.keys[node.right_idx], node.values[node.right_idx]);
			ret.node = node;
	...
	K removeLeftmost(Node<K, V> node) {
		...
		if (node.size == 1) {
			deleteNode(node);
				...
				&& (Node.NODE_SIZE - 1 - node.prev.right_idx) > node.size) {
			// move all to prev node and kill it
			Node<K, V> prev = node.prev;
			int size = node.right_idx - index;
			System.arraycopy(node.keys, index + 1, prev.keys,
					prev.right_idx + 1, size);
			System.arraycopy(node.values, index + 1, prev.values,
					prev.right_idx + 1, size);
			prev.right_idx += size;
			prev.size += size;
			deleteNode(node);
			...
			System.arraycopy(node.keys, index + 1, next.keys, next_new_left,
					size);
			System.arraycopy(node.values, index + 1, next.values,
					next_new_left, size);
			next.size += size;
			deleteNode(node);
		} else {
	...
	K removeRightmost(Node<K, V> node) {
		...
		K key = (node != null && node.next != null) ? node.next.keys[node.next.left_idx]
				: null;
		if (node.size == 1) {
			deleteNode(node);
		} else if (node.prev != null
			...
			System.arraycopy(node.keys, left_idx, prev.keys,
					prev.right_idx + 1, size);
			System.arraycopy(node.values, left_idx, prev.values,
					prev.right_idx + 1, size);
			prev.right_idx += size;
			prev.size += size;
			deleteNode(node);
			...
			System.arraycopy(node.keys, left_idx, next.keys, next_new_left,
					size);
			System.arraycopy(node.values, left_idx, next.values, next_new_left,
					size);
			next.size += size;
			deleteNode(node);
		} else {
	...
	K removeMiddleElement(Node<K, V> node, int index) {
		...
		if (node.prev != null
			...
			if (moveFromRight <= moveFromLeft) {
				System.arraycopy(node.keys, index + 1, node.keys, index,
						moveFromRight);
				System.arraycopy(node.values, index + 1, node.values, index,
						moveFromRight);
				Node<K, V> next = node.next;
				if (next != null && next.size == 1) {
					node.keys[node.right_idx] = next.keys[next.left_idx];
				...
				System.arraycopy(node.keys, left_idx, node.keys, left_idx + 1,
						moveFromLeft);
				System.arraycopy(node.values, left_idx, node.values,
			...
			entrySet = new AbstractSet<Map.Entry<K, V>>() {
				...
				public boolean contains(Object object) {
					if (object instanceof Map.Entry) {
						...
						return v1 == null ? (v2 == null && TreeMap.this
								.containsKey(key)) : v1.equals(v2);
					}
					return false;
				}
	...
	static class UnboundedEntryIterator<K, V> extends AbstractMapIterator<K, V>
		...
		public Map.Entry<K, V> next() {
			makeNext();
			int idx = lastOffset;
			return new TreeMapEntry<K, V>(lastNode.keys[idx],
					lastNode.values[idx], lastNode, idx);
		}
	}
