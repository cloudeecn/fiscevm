 ...
 * {@code String} is represented by array of UTF-16 values, such that Unicode
 * supplementary characters (code points) are stored/encoded as surrogate pairs
 * via Unicode code units ({@code char}).
 * 
 * @see StringBuffer
 * @see StringBuilder
 * @see Charset
...
public final class String implements Comparable<String>, CharSequence {
	...
	public String(byte[] data, int start, int length) {
		value = FiScEVM.decode(data, start, length);
		offset = 0;
		count = value.length;
	}
	...
	public String(byte[] data, int start, int length, final String encoding)
			throws UnsupportedEncodingException {
		value = FiScEVM.decode(encoding, data, start, length);
		offset = 0;
		count = value.length;
	}
	...
	public byte[] getBytes(String encoding) throws UnsupportedEncodingException {
		return FiScEVM.encode(encoding, value, offset, count);
	}
	 ...
	 * is null return the string {@code "null"}, otherwise use
	 ...
	 * boolean is {@code true} return {@code "true"}, otherwise return
	 * {@code "false"}.
	 * 
	 * @param value
	 *            the boolean.
	 ...
	 *             or if there aren't enough code points before or after
	 *             {@code index} to match {@code codePointOffset}.
	 * @since 1.5
	 */
