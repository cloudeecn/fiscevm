...
public class DefaultThreadManager implements Runnable, JThreadManager {
	// constant
	...
	// *********************
	// persist
	private final ArrayList<DefaultThread> runningThreads = new ArrayList<DefaultThread>();
	...
	private final ArrayList<DefaultThread> pendingThreads = new ArrayList<DefaultThread>();
	private long nextWakeUpTimeTotal;
	private int nextThreadId;
	private Throwable exitException;
	private int exitCode;

	public void monitorEnter(DefaultThread dt, int monitorId) {
		monitorEnter(dt, monitorId, 1);
	}

	public void monitorExit(DefaultThread dt, int monitorId)
			throws VMException, VMCriticalException {
		monitorExit(dt, monitorId, 1);
	}

	public void sleep(IThread dt, long time) {
		int threadId = ((DefaultThread) dt).getThreadId();
		nextWakeUpTime[threadId] = System.currentTimeMillis() + time;
		((DefaultThread) dt).setYield(true);
	}

	public void interrupt(int targetHandle) throws VMException,
			VMCriticalException {
		DefaultThread target = getThread(targetHandle);
		if (target == null) {
			return;
		}
	...
	public void wait(IThread thread, int monitorId, long time)
			throws VMException, VMCriticalException {
		DefaultThread dt = (DefaultThread) thread;
		...
		((DefaultThread) thread).setYield(true);
	}
	...
	public boolean isAlive(int threadHandle) {
		DefaultThread t = getThread(threadHandle);
		return t != null;
	}

	public void destroyThread(DefaultThread dt) {
	...
	private void monitorEnter(DefaultThread dt, int monitorId, int times) {
	...
	private void monitorExit(DefaultThread dt, int monitorId, int times)
			throws VMException, VMCriticalException {
		int threadId = dt.getThreadId();
	...
	private int releaseMonitor(DefaultThread dt, int monitorId)
			throws VMException, VMCriticalException {
		int threadId = dt.getThreadId();
		if (monitorOwnerId[monitorId] != threadId) {
			throw new VMException("java/lang/IllegalMonitorStateException", "");
	...
	private DefaultThread getThread(int threadHandle) {
		synchronized (threadsLock) {
	...
	private void onThreadAdd(DefaultThread dt) {

	}

	private void onThreadRemove(DefaultThread dt) {
		int tid = dt.getThreadId();
		// release all locks
		waitForLockId[tid] = 0;
	...
	public void setPriority(int threadHandle, int priority) throws VMException {
		DefaultThread dt = getThread(threadHandle);
		if (dt == null) {
	...
	public void saveData(Element data) {
		// private final ArrayList<DefaultThread> runningThreads = new
		// ArrayList<DefaultThread>();
		// thread id --> monitor id waiting for monitorentry
		// private int[] waitForLockId = new int[MAX_THREADS];
		// thread id --> monitor id waiting for notify
		// private int[] waitForNotifyId = new int[MAX_THREADS];
